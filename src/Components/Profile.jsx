...
import { Navbar } from "./";
const Profile = () => {
  const { friendList, sidebarToShow, startNewChat, setOpenedChat, setMessage, user } =
    useStateContext();
  ...
  return (
    <>
      ...
      />
    {user &&  <div className="flex flex-col relative w-full justify-center  mt-32">
        {/* image */}
        ...
        {user.profileImage.length ? (
              ...
              rounded-full relative"
                />)}
                <div style={{backgroundColor:'rgba(0,0,0,0.8)'}} className="absolute h-full w-full top-0 left-0  cursor-pointer rounded-full  hover:opacity-100 flex transition-opacity opacity-0 justify-center items-center flex-col">
                  <AiOutlineCamera className="text-slate-100 cursor-pointer text-2xl mb-2"/>
                  <p className="text-small dark:text-slate-100 uppercase text-center">Change your profile picture</p>
                </div>
        </div></div>
        {/* name */}
        <div className="mt-4 py-4 flex flex-col px-6">
          <p className="text-teal-600 text-small">
            Your name
          </p>
        ...
        <div className="flex w-full px-6 py-4">
          <p className="dark:text-slate-400 text-sm">This is not your username or pin. This name will be visible to your Whatsapp contacts</p>
        </div>
        <div className="mt-4 py-4 flex flex-col px-6">
          <p className="text-teal-600 text-small">
           About
